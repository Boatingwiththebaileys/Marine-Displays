...
bool imu_is_available(void) { return g_imu_available; }
...
bool getAccelerometer(void)
{
    uint8_t buf[6];
    if (I2C_Read(Device_addr, QMI8658_AX_L, buf, 6) != 0) return false;
...
bool getGyroscope(void)
{
    uint8_t buf[6];
    if (I2C_Read(Device_addr, QMI8658_GX_L, buf, 6) != 0) return false;
